	...
	class WaspNest : ModItem
	{
		...
		private void HookBeeType(ILContext il)
		{
            ...
            if (implementation == 0)
			{
				...
				{
					// Regular c# code
					if (player.GetModPlayer<HumanPlayer>().strongBeesUpgrade && Main.rand.NextBool(10) && Main.ProjectileUpdateLoopIndex == -1)
			...
			else if (implementation == 1)
			{
				...
				c.EmitDelegate<Func<Player, bool>>(player => player.GetModPlayer<HumanPlayer>().strongBeesUpgrade && Main.rand.NextBool(10) && Main.ProjectileUpdateLoopIndex == -1);
			...
			else
			{
				...
				c.Emit(OpCodes.Call, typeof(Player).GetMethod("GetModPlayer", new Type[] { }).MakeGenericMethod(typeof(HumanPlayer)));
				// nameof helps avoid spelling mistakes
				c.Emit(OpCodes.Ldfld, typeof(HumanPlayer).GetField(nameof(HumanPlayer.strongBeesUpgrade)));
		...
		public override void UpdateAccessory(Player player, bool hideVisual)
		{
			...
			player.GetModPlayer<HumanPlayer>().strongBeesUpgrade = true;
		}
	}
}
